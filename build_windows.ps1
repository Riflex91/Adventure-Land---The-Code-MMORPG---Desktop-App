...
cmake -S $Root -B (Join-Path $Root "build-win") -A x64 -DCMAKE_TOOLCHAIN_FILE=$Toolchain -DVCPKG_TARGET_TRIPLET=x64-windows -DAL_BUILD_TESTS=ON
...
Copy-Item (Join-Path $Root "build-win\Release\adventure-land-reference-os.exe") $Dist -Force
Copy-Item (Join-Path $Root "resources\reference.db") (Join-Path $Dist "resources\reference.db") -Force
Write-Host "Built: $Dist\adventure-land-reference-os.exe"
